  ...
  Fix(lmp, narg, arg)
	{
		...
		if(parse) parse_args(narg,arg); //True//
	}

/************************Commented in style (//  //) is in reference to add breakparticle feature***************************/

void FixPropertyAtom::parse_args(int narg, char **arg)
	{
		// Check args
		if (narg < 9) error->all(FLERR,"Illegal fix property/atom command, not enough arguments");		//9 in our case//
		...
		strcpy(variablename,arg[3]);    //break_id//

		if (strcmp(arg[4],"scalar") == 0) data_style = FIXPROPERTY_ATOM_SCALAR;		//scalar//
		...
		if (strcmp(arg[5],"yes") == 0) restart_peratom = 1;		//yes//
		...
		if (strcmp(arg[6],"yes") == 0) commGhost = 1;		//yes//
		...
		else if (strcmp(arg[7],"no") == 0) commGhostRev = 0;	//no//
		else error->all(FLERR,"Unknown communicate_reverse_ghost style for fix property/atom. Valid styles are 'yes' or 'no'");

		nvalues = narg - 8;		//nvalues=1//
		if ((nvalues == 1) && (data_style != FIXPROPERTY_ATOM_SCALAR))		//fails//
		...
		defaultvalues = new double[nvalues];	//new double[1]//

		// fix handles properties that need to be initialized at particle creation
		create_attribute = 1;
		for (int j = 0; j < nvalues; j++)		//navlues=1//j=0//
		{
			// if any of the values is none, this fix will not init properties
			if(strcmp(arg[8+j],"none") == 0)		//Not true in our case//
				{
					create_attribute = 0;
					continue;
				}
			defaultvalues[j] = myAtof(arg[8+j]); 	//defaultvalues[0]=0//because arg[8] which is 0//
		}

		if (data_style) size_peratom_cols = nvalues;	//data_style is scalar//So, size_peratom_cols=1//
		...
		if (commGhost) comm_forward = nvalues;	//True//comm_forward=1//
		if (commGhostRev) comm_reverse = nvalues;	//False//

		// perform initial allocation of atom-based array
		// register with Atom class
		vector_atom = NULL; array_atom = NULL;
		grow_arrays(atom->nmax); 	//declared in atom.h//nmax is maximum no of owned+ghost in arrays on this proc//
									//function defined in this file itself//grows memory to hold atoms data to appropriate value//
		atom->add_callback(0); 		//grows memory//
		if (restart_peratom) atom->add_callback(1); 	//grows memory//

		// init all arrays since dump may access it on timestep 0
		// or a variable may access it before first run
		
		int nlocal = atom->nlocal;	//nlocal is no of atoms in current processor//
			...
			for (int i = 0; i < nlocal; i++)
			{
			  if (data_style)	//true//
			  {
				for (int m = 0; m < nvalues; m++)
					array_atom[i][m] = defaultvalues[m];	//assigning default values 0 to all//
			  }
